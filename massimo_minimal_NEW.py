...
#        numpy.savetxt(file, tot, delimiter=",", fmt='%1.0f')
        numpy.savetxt(file, delimiter=",", fmt='%1.0f')
...
def loadTime(link, timemax):
    ...
    if "https://" in link: #Se è un vero link (http://)
        while True:
            try:
                ...
                time.sleep(10)
            break
    elif "#" in link: # Se non è un vero link, è un percorso con css
        time.sleep(3)
        try:
    ...
    while True:
        ...
        mat.append([rect, 0, current])
        print("Rect:" + str(rect) + " - " + str(current))
...
def lastLoadTime(link, ricalcolo, timemax):
    ...
    for c in range(int(ricalcolo)):  # per un max di tot volte
        ...
        elif tempo < 200:  # se il tempo calcolato è < 200
...
#la differenza con il massimo minimal è il flag