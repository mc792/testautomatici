    ...
    for j in range(int(giri)):
        ...
        if j != (giri-1): #Se non è l'ultimo giro fai aspettare altrimenti finisci
            while (current_milli_time() - aspettatime) < aspetta:
                print("Mancano " + str(round((aspetta - (current_milli_time() - aspettatime)) / 1000)) + " secondi al giro " + str(j+2))
                time.sleep(1)
    print (vettoregiri)
...
def inizia():
    ...
    aspetta = verificaNumero("Quanto vuoi aspettare fra un giro e l'altro? (consigliato 10000): ")
    i = datetime.datetime.now()
    dt = "%s-%s-%s - %s.%s.%s" % (i.year, i.month, i.day, i.hour, i.minute, i.second)
    base = "TEST/"+dt

    lista = [
        ["Dashboard", "https://stage.new.sitemanager.ability.abb/#/"]
    ]
...
#modifiche da stageW2:
#ritardo di 300ms sul controllo (riga 162)

#modifiche da stageW2_2:
#controllo sul return current (righe 163-167)
#controllo sul tempo (206-207)

#ULTIMA VERSIONE AD ORA

#NELLA PROSSIMA VERSIONE PROVARE AD ALLUNGARE IL PERIODO DI FLAG (RIGHE 175-181)
